...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    -> StateT s f a
    -> StateT s f b
  (<*>) f a = StateT (\s -> runStateT f s >>= (\(f', s') -> first f' <$> runStateT a s'))
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' f = runExactlyOne . runStateT f
...
instance Monad f => Monad (OptionalT f) where
  (=<<) f a = OptionalT (runOptionalT a >>= \oa -> case oa of
    Empty -> pure Empty
